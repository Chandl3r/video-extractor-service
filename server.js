...
process.on('unhandledRejection', (r) => console.error('[v96] unhandledRejection:', r?.message || r));

app.get('/', (req, res) => res.json({ status: 'ok', service: 'Video Extractor v96' }));
...
function closeSession() {
    if (session) {
        console.log('[v96] Chiudo sessione');
...
app.post('/extract', async (req, res) => {
    ...
    if (session && session.embedUrl === url) {
        session.ts = Date.now();
        console.log('[v96] Cache hit:', session.videoUrl.substring(0, 60));
        return res.json({ success: true, video_url: session.videoUrl });
    }

    closeSession();
    console.log('[v96] ESTRAZIONE:', url);
    let browser = null, page = null, resolved = false;

    const globalTimeout = setTimeout(() => {
        console.log('[v96] TIMEOUT');
        if (!resolved) {
            resolved = true;
    ...
    try {
        ...
        page.on('request', (request) => {
            ...
            if (looksLikeVideo(u)) {
                console.log('[v96] Video:', u.substring(0, 80));
                    ...
                    page.goto('about:blank', { waitUntil: 'domcontentloaded', timeout: 5000 })
                        .then(async () => {
                            try {
                                ...
                                console.log('[v96] ✅ CDP pronto');
                                session = { embedUrl: url, videoUrl: u, browser, page, cdp, ts: Date.now(), chunkCount: 0 };
                                res.json({ success: true, video_url: u });
                                console.log('[v96] → Risposta inviata, session salvata');
                            } catch(e) {
                                console.error('[v96] CDP err:', e.message);
                                if (browser) browser.close().catch(() => {});
                        ...
                        .catch(() => {
                            setTimeout(async () => {
                                try {
                                    ...
                                    session = { embedUrl: url, videoUrl: u, browser, page, cdp, ts: Date.now(), chunkCount: 0 };
                                    res.json({ success: true, video_url: u });
                                    console.log('[v96] → Risposta inviata (fallback)');
                                } catch(e) {
                                    if (browser) browser.close().catch(() => {});
        ...
        await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 15000 })
            .catch(e => console.log('[v96] goto:', e.message.substring(0, 60)));
        ...
        for (let w = 0; w < 10 && !resolved; w++) {
            ...
            if (q && !resolved) {
                ...
                try {
                    ...
                    console.log('[v96] → Risposta inviata (poll loop)');
                } catch(e) {
                    if (browser) browser.close().catch(() => {});
            ...
            if ((i+1) % 3 === 0) {
                ...
                if (v && !resolved) {
                    ...
                    try {
                        ...
                        console.log('[v96] → Risposta inviata (click loop)');
                    } catch(e) {
                        if (browser) browser.close().catch(() => {});
                        res.json({ success: false, message: 'CDP err: ' + e.message });
                    }
                ...
                console.log(`[v96] Click ${i+1}: niente`);
            }
        }
    } catch(e) {
        console.error('[v96] ERRORE:', e.message);
...
app.get('/proxy', async (req, res) => {
    ...
    const CHUNK = 512 * 1024;  // 512KB: doppio di v92, max dati prima di hit limite connessioni
    ...
    try {
        await withProxyLock(async () => {
            ...
            if (session) {
                ...
                // Ogni 8 chunk: reload leggero per liberare fetch appese nel renderer
                // data:text/html, è più leggero di about:blank (zero risorse)
                // CDP session rimane attiva (è a livello browser target, non page)
                if (session.chunkCount % 8 === 0) {
                    console.log(`[proxy] 🔄 Reload leggero (chunk ${session.chunkCount})`);
                    await page.goto('data:text/html,', { waitUntil: 'domcontentloaded', timeout: 2000 }).catch(() => {});
                    session.chunkCount = 0; // Reset counter
                }
            }
        });
    } catch(e) {
...
app.listen(PORT, () => console.log(`Video Extractor v96 porta ${PORT}`));
