...
process.on('unhandledRejection', (r) => console.error('[v132] unhandledRejection:', r?.message || r));

app.get('/', (req, res) => res.json({ status: 'ok', service: 'Video Extractor v132' }));
...
function closeSession() {
    if (session) {
        console.log('[v132] Chiudo sessione');
...
app.post('/extract', async (req, res) => {
    ...
    if (session && session.embedUrl === url) {
        session.ts = Date.now();
        console.log('[v132] Cache hit:', session.videoUrl.substring(0, 60));
        return res.json({ success: true, video_url: session.videoUrl });
    }

    closeSession();
    console.log('[v132] ESTRAZIONE:', url);
    let browser = null, page = null, resolved = false;

    const globalTimeout = setTimeout(() => {
        console.log('[v132] TIMEOUT');
        if (!resolved) {
            resolved = true;
    ...
    try {
        ...
        page.on('request', (request) => {
            ...
            if (looksLikeVideo(u)) {
                console.log('[v132] Video:', u.substring(0, 80));
                ...
                if (!resolved) {
                    ...
                    (async () => {
                        try {
                            ...
                            console.log('[v132] ✅ CDP pronto');
                            session = { embedUrl: url, videoUrl: u, browser, page, cdp, ts: Date.now(), chunkCount: 0 };
                            res.json({ success: true, video_url: u });
                            console.log('[v132] → Risposta inviata, session salvata');
                        } catch(e) {
                            console.error('[v132] CDP err:', e.message);
                            if (browser) browser.close().catch(() => {});
        ...
        await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 15000 })
            .catch(e => console.log('[v132] goto:', e.message.substring(0, 60)));
        ...
        for (let w = 0; w < 10 && !resolved; w++) {
            ...
            if (q && !resolved) {
                ...
                try {
                    ...
                    console.log('[v132] → Risposta inviata (poll loop)');
                } catch(e) {
                    if (browser) browser.close().catch(() => {});
            ...
            if ((i+1) % 3 === 0) {
                ...
                if (v && !resolved) {
                    ...
                    try {
                        ...
                        console.log('[v132] → Risposta inviata (click loop)');
                    } catch(e) {
                        if (browser) browser.close().catch(() => {});
                        res.json({ success: false, message: 'CDP err: ' + e.message });
                    }
                ...
                console.log(`[v132] Click ${i+1}: niente`);
            }
        }
    } catch(e) {
        console.error('[v132] ERRORE:', e.message);
...
app.listen(PORT, () => console.log(`Video Extractor v132 porta ${PORT}`));
