                        ...
                        .catch(() => {
                            setTimeout(async () => {
                                try {
                                    ...
                                    session = { embedUrl: url, videoUrl: u, browser, page, cdp, ts: Date.now() };
                                    res.json({ success: true, video_url: u });
                                    console.log('[v117] → Risposta inviata (fallback)');
                                } catch(e) {
                                    if (browser) browser.close().catch(() => {});
...
app.get('/proxy', async (req, res) => {
    ...
    try {
        await withProxyLock(async () => {
            ...
            // IMPORTANTE: usa sempre session.cdp, mai let { cdp }
            // session.cdp viene aggiornato dopo reset → i listener devono sempre puntare all'attuale
            const streamReady = new Promise((resolve, reject) => {
                const timer = setTimeout(() => {
                    session.cdp.removeListener('Fetch.requestPaused', handler);
                    reject(new Error('Timeout CDP 25s'));
                }, 25000);

                const handler = async (ev) => {
                    if (ev.responseStatusCode === undefined) {
                        await session.cdp.send('Fetch.continueRequest', { requestId: ev.requestId }).catch(() => {}); return;
                    }
                    clearTimeout(timer);
                    session.cdp.removeListener('Fetch.requestPaused', handler);
                    ...
                    if (status >= 400) {
                        await session.cdp.send('Fetch.continueRequest', { requestId: ev.requestId }).catch(() => {});
                        reject(new Error(`HTTP ${status}`)); return;
                    }
                    try {
                        const { stream } = await session.cdp.send('Fetch.takeResponseBodyAsStream', { requestId: ev.requestId });
            ...
            let total = 0;
            while (true) {
                const chunk = await session.cdp.send('IO.read', { handle: stream, size: 65536 });
            ...
            res.end();
            await session.cdp.send('IO.close', { handle: stream }).catch(() => {});
