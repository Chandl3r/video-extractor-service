...
process.on('unhandledRejection', (r) => console.error('[v129] unhandledRejection:', r?.message || r));

app.get('/', (req, res) => res.json({ status: 'ok', service: 'Video Extractor v129' }));
...
function closeSession() {
    if (session) {
        console.log('[v129] Chiudo sessione');
...
app.post('/extract', async (req, res) => {
    ...
    if (session && session.embedUrl === url) {
        session.ts = Date.now();
        console.log('[v129] Cache hit:', session.videoUrl.substring(0, 60));
        return res.json({ success: true, video_url: session.videoUrl });
    }

    closeSession();
    console.log('[v129] ESTRAZIONE:', url);
    let browser = null, page = null, resolved = false;

    const globalTimeout = setTimeout(() => {
        console.log('[v129] TIMEOUT');
        if (!resolved) {
            resolved = true;
    ...
    try {
        ...
        page.on('request', (request) => {
            ...
            if (looksLikeVideo(u)) {
                console.log('[v129] Video:', u.substring(0, 80));
                    ...
                    page.goto('about:blank', { waitUntil: 'domcontentloaded', timeout: 5000 })
                        .then(async () => {
                            try {
                                ...
                                console.log('[v129] CDP session created');
                                await cdp.send('Fetch.enable', { patterns: [{ urlPattern: '*mxcontent.net*', requestStage: 'Response' }] });
                                console.log('[v129] Fetch.enable done');
                                session = { embedUrl: url, videoUrl: u, browser, page, cdp, ts: Date.now(), chunkCount: 0 };
                                console.log('[v129] Session assigned');
                                res.json({ success: true, video_url: u });
                                console.log('[v129] Response sent');
                                // ATTESA per vedere se crasha qui o continua
                                await new Promise(resolve => setTimeout(resolve, 100));
                                console.log('[v129] Still alive after 100ms');
                            } catch(e) {
                                console.error('[v129] CDP err:', e.message);
                                if (browser) browser.close().catch(() => {});
                                res.json({ success: false, message: 'CDP err' });
                            }
                        })
                        .catch((err) => {
                            console.error('[v129] ⚠️ Main path failed:', err?.message || err);
                            console.error('[v129] Stack:', err?.stack);
                            setTimeout(async () => {
                                console.log('[v129] Attempting fallback...');
                                try {
                                    const cdp = await page.target().createCDPSession();
                                    console.log('[v129] Fallback: CDP created');
                                    await cdp.send('Fetch.enable', { patterns: [{ urlPattern: '*mxcontent.net*', requestStage: 'Response' }] });
                                    console.log('[v129] Fallback: Fetch.enable done');
                                    session = { embedUrl: url, videoUrl: u, browser, page, cdp, ts: Date.now(), chunkCount: 0 };
                                    console.log('[v129] Fallback: Session assigned');
                                    res.json({ success: true, video_url: u });
                                    console.log('[v129] Fallback: Response sent');
                                } catch(e) {
                                    console.error('[v129] Fallback error:', e.message);
                                    if (browser) browser.close().catch(() => {});
        ...
        await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 15000 })
            .catch(e => console.log('[v129] goto:', e.message.substring(0, 60)));
        ...
        for (let w = 0; w < 10 && !resolved; w++) {
            ...
            if (q && !resolved) {
                ...
                try {
                    ...
                    console.log('[v129] → Risposta inviata (poll loop)');
                } catch(e) {
                    if (browser) browser.close().catch(() => {});
            ...
            if ((i+1) % 3 === 0) {
                ...
                if (v && !resolved) {
                    ...
                    try {
                        ...
                        console.log('[v129] → Risposta inviata (click loop)');
                    } catch(e) {
                        if (browser) browser.close().catch(() => {});
                        res.json({ success: false, message: 'CDP err: ' + e.message });
                    }
                ...
                console.log(`[v129] Click ${i+1}: niente`);
            }
        }
    } catch(e) {
        console.error('[v129] ERRORE:', e.message);
...
app.listen(PORT, () => console.log(`Video Extractor v129 porta ${PORT}`));
