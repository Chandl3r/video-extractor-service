                        ...
                        .catch(() => {
                            setTimeout(async () => {
                                try {
                                    ...
                                    session = { embedUrl: url, videoUrl: u, browser, page, cdp, ts: Date.now() };
                                    res.json({ success: true, video_url: u });
                                    console.log('[v118] → Risposta inviata (fallback)');
                                } catch(e) {
                                    if (browser) browser.close().catch(() => {});
...
app.get('/proxy', async (req, res) => {
    ...
    try {
        await withProxyLock(async () => {
            ...
            const { page } = session;
            // Cattura CDP all'inizio: questo chunk usa QUESTO CDP per tutta la sua vita
            // Anche se session.cdp viene aggiornato da un reset concorrente, questo handler
            // continua a usare il suo CDP catturato
