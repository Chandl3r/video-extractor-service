...
process.on('unhandledRejection', (r) => console.error('[v91] unhandledRejection:', r?.message || r));

app.get('/', (req, res) => res.json({ status: 'ok', service: 'Video Extractor v91' }));
...
function closeSession() {
    if (session) {
        console.log('[v91] Chiudo sessione');
...
app.post('/extract', async (req, res) => {
    ...
    if (session && session.embedUrl === url) {
        session.ts = Date.now();
        console.log('[v91] Cache hit:', session.videoUrl.substring(0, 60));
        return res.json({ success: true, video_url: session.videoUrl });
    }

    closeSession();
    console.log('[v91] ESTRAZIONE:', url);
    let browser = null, page = null, resolved = false;

    const globalTimeout = setTimeout(() => {
        console.log('[v91] TIMEOUT');
        if (!resolved) {
            resolved = true;
    ...
    try {
        ...
        page.on('request', (request) => {
            ...
            if (looksLikeVideo(u)) {
                console.log('[v91] Video:', u.substring(0, 80));
                ...
                if (!resolved) {
                    ...
                    page.goto('about:blank', { waitUntil: 'domcontentloaded', timeout: 5000 })
                        .then(async () => {
                            try {
                                // GC forzato: alloca 50MB poi lascia che V8 liberi tutto
                                await page.evaluate(() => {
                                    const arr = new Array(50 * 1024 * 1024 / 8).fill(1);
                                    arr.length = 0;
                                }).catch(() => {});
                                await sleep(500); // Attende GC
                                const cdp = await page.target().createCDPSession();
                                await cdp.send('Fetch.enable', {
                                    patterns: [{ urlPattern: '*mxcontent.net*', requestStage: 'Response' }]
                                });
                                console.log('[v91] ✅ CDP pronto');
                                session = { embedUrl: url, videoUrl: u, browser, page, cdp, ts: Date.now() };
                                res.json({ success: true, video_url: u });
                            } catch(e) {
                                console.error('[v91] CDP err:', e.message);
                                if (browser) browser.close().catch(() => {});
        ...
        await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 15000 })
            .catch(e => console.log('[v91] goto:', e.message.substring(0, 60)));
        ...
        for (let i = 0; i < 25 && !resolved; i++) {
            ...
            if ((i+1) % 3 === 0) {
                ...
                console.log(`[v91] Click ${i+1}: niente`);
            }
        }
    } catch(e) {
        console.error('[v91] ERRORE:', e.message);
...
app.get('/proxy', async (req, res) => {
    ...
    try {
        await withProxyLock(async () => {
            ...
            console.log(`[proxy] CDP fetch: ${rangeStr} (waited ${lockAcquired - reqStart}ms for lock)`);
...
app.listen(PORT, () => console.log(`Video Extractor v91 porta ${PORT}`));
