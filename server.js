...
try { 
    // Kill aggressivo processi Chrome zombie - 3 tentativi
    execSync('pkill -9 -f "chromium|chrome" 2>/dev/null || true', { timeout: 2000 }); 
    execSync('sleep 0.5', { timeout: 1000 });
    execSync('pkill -9 -f "chromium|chrome" 2>/dev/null || true', { timeout: 2000 }); 
    execSync('sleep 0.5', { timeout: 1000 });
    // Pulisci /tmp da file Chrome vecchi
    execSync('rm -rf /tmp/.X* /tmp/.org.chromium.* /tmp/puppeteer* 2>/dev/null || true', { timeout: 3000 });
    console.log('[v125] ✅ Cleanup completato');
} catch(e) { console.log('[v125] Cleanup err:', e.message); }
...
async function launchBrowser() {
    console.log('[v125] Launching browser...');
    const browser = await puppeteer.launch({
        ...
        timeout: 20000, // 20s invece di 30s
    });
    console.log('[v125] ✅ Browser launched');
    return browser;
}
                    ...
                    page.goto('about:blank', { waitUntil: 'domcontentloaded', timeout: 5000 })
                        .then(async () => {
                            try {
                                ...
                                // Sleep 100ms: garantisce che session.cdp sia completamente stabile
                                // prima che il client riceva video_url e faccia richiesta /proxy
...
app.get('/proxy', async (req, res) => {
    ...
    console.log('[proxy] RICHIESTA RICEVUTA');
    ...
    console.log('[proxy] Session.cdp exists:', !!session?.cdp);
