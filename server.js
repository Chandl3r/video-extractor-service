...
process.on('unhandledRejection', (r) => console.error('[v99] unhandledRejection:', r?.message || r));

app.get('/', (req, res) => res.json({ status: 'ok', service: 'Video Extractor v99' }));
...
function closeSession() {
    if (session) {
        console.log('[v99] Chiudo sessione');
...
app.post('/extract', async (req, res) => {
    ...
    if (session && session.embedUrl === url) {
        session.ts = Date.now();
        console.log('[v99] Cache hit:', session.videoUrl.substring(0, 60));
        return res.json({ success: true, video_url: session.videoUrl });
    }

    closeSession();
    console.log('[v99] ESTRAZIONE:', url);
    let browser = null, page = null, resolved = false;

    const globalTimeout = setTimeout(() => {
        console.log('[v99] TIMEOUT');
        if (!resolved) {
            resolved = true;
    ...
    try {
        ...
        page.on('request', (request) => {
            ...
            if (looksLikeVideo(u)) {
                console.log('[v99] Video:', u.substring(0, 80));
                    ...
                    page.goto('about:blank', { waitUntil: 'domcontentloaded', timeout: 5000 })
                        .then(async () => {
                            try {
                                ...
                                console.log('[v99] ✅ CDP pronto');
                                session = { embedUrl: url, videoUrl: u, browser, page, cdp, ts: Date.now(), chunkCount: 0 };
                                res.json({ success: true, video_url: u });
                                console.log('[v99] → Risposta inviata, session salvata');
                            } catch(e) {
                                console.error('[v99] CDP err:', e.message);
                                if (browser) browser.close().catch(() => {});
                        ...
                        .catch(() => {
                            setTimeout(async () => {
                                try {
                                    ...
                                    session = { embedUrl: url, videoUrl: u, browser, page, cdp, ts: Date.now(), chunkCount: 0 };
                                    res.json({ success: true, video_url: u });
                                    console.log('[v99] → Risposta inviata (fallback)');
                                } catch(e) {
                                    if (browser) browser.close().catch(() => {});
        ...
        await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 15000 })
            .catch(e => console.log('[v99] goto:', e.message.substring(0, 60)));
        ...
        for (let w = 0; w < 10 && !resolved; w++) {
            ...
            if (q && !resolved) {
                ...
                try {
                    ...
                    console.log('[v99] → Risposta inviata (poll loop)');
                } catch(e) {
                    if (browser) browser.close().catch(() => {});
            ...
            if ((i+1) % 3 === 0) {
                ...
                if (v && !resolved) {
                    ...
                    try {
                        ...
                        console.log('[v99] → Risposta inviata (click loop)');
                    } catch(e) {
                        if (browser) browser.close().catch(() => {});
                        res.json({ success: false, message: 'CDP err: ' + e.message });
                    }
                ...
                console.log(`[v99] Click ${i+1}: niente`);
            }
        }
    } catch(e) {
        console.error('[v99] ERRORE:', e.message);
...
app.get('/proxy', async (req, res) => {
    ...
    const CHUNK = 1024 * 1024;  // 1MB: ~14 fetch = 14MB totali prima del limite Chrome
    ...
    try {
        await withProxyLock(async () => {
            ...
            if (session) {
                ...
                // Reload al chunk 7: libera fetch prima del limite Chrome (~14-15)
                // waitUntil:'commit' è più veloce di 'domcontentloaded' (commit = navigazione committata ma non caricata)
                // Riduce race window con seek video da ~200ms a ~50ms
                if (session.chunkCount % 7 === 0) {
                    console.log(`[proxy] 🔄 Reload veloce (chunk ${session.chunkCount})`);
                    await page.goto('data:text/html,', { waitUntil: 'commit', timeout: 1000 }).catch(() => {});
                }
            }
        });
    } catch(e) {
...
app.listen(PORT, () => console.log(`Video Extractor v99 porta ${PORT}`));
